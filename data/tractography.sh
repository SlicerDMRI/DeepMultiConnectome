...
process_subject() {
    ...
    if [ ! -f ${dwi_meanbzero_brain_mask} ]; then
        ...
        # Use MRtrix dwi2mask instead of FSL BET (FSL not required in environment)
        dwi2mask "${dwi_mif}" "${dwi_meanbzero_brain_mask}" 2>&1 | tee -a "${log_file}"
        
        # Create brain-extracted image (multiply mean b0 by mask)
        mrcalc "${dwi_meanbzero_nii}" "${dwi_meanbzero_brain_mask}" -mult "${dwi_meanbzero_brain}" 2>&1 | tee -a "${log_file}"
    fi
    ...
    # Check if standard2acpc_dc.nii.gz exists (TractoInferno doesn't have this file)
    if [ -f "${anat_dir}/standard2acpc_dc.nii.gz" ] && [ ! -f ${tracts_MNI} ]; then
        echo -e "${GREEN}[INFO]${NC} `date`: Transforming tracts to MNI space" | tee -a "${log_file}"
        ...
        echo -e "${GREEN}[INFO]${NC} `date`: Skipping MNI transformation (standard2acpc_dc.nii.gz not found)" | tee -a "${log_file}"
    fi
    ...
    if [ -f ${tracts_MNI} ] && [ ! -f ${tracts_vtk_MNI} ]; then
        tckconvert -binary ${tracts_MNI} ${tracts_vtk_MNI} 2>&1 | tee -a "${log_file}" 
    fi

    # Define parcellations - check which ones exist
    parcellations=()
    if [ -f "${anat_dir}/aparc+aseg.nii.gz" ]; then
        parcellations+=("aparc+aseg")
    fi
    if [ -f "${anat_dir}/aparc.a2009s+aseg.nii.gz" ]; then
        parcellations+=("aparc.a2009s+aseg")
    fi
    
    # If no parcellations found, log warning and skip
    if [ ${#parcellations[@]} -eq 0 ]; then
        echo -e "${RED}[WARNING]${NC} `date`: No parcellation files found for ${subject_id}, skipping connectome generation" | tee -a "${log_file}"
        return
    fi

    # Loop over available parcellations
